...
class MigrationExecutor:
    ...
    def _debug_states(self, state, app_list=["squashme", "triggerfailingcode"]):
        for app_label, app_state in state.apps.app_configs.items():
            if app_label in app_list:
                print(f"[STATE] App: {app_label}")
    ...
    def _create_project_state(self, with_applied_migrations=False):
        ...
        if with_applied_migrations:
            ...
            print(f"******[DEBUG] _create_project_state full plan: {[(i[0].app_label, i[0].name) for i in full_plan if i[0].app_label in ["triggerfailingcode", "squashme"]]}")
            applied_migrations = {
        ...
        print(f"[STATE] _create_project_state")
        self._debug_states(state)
        return state
    ...
    def _migrate_all_backwards(self, plan, full_plan, fake):
        ...
        the plan.
        """
        print(f"******[DEBUG] _migrate_all_backwards full plan: {[(i[0].app_label, i[0].name) for i in full_plan if i[0].app_label in ["triggerfailingcode", "squashme"]]}")
        migrations_to_run = {m[0] for m in plan}
        ...
        print(f"[STATE] _migrate_all_backwards")
        self._debug_states(state)
